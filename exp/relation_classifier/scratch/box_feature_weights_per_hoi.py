...
def main():
    ...
    model_const.geometric_per_hoi = True
    ...
    weight = model.geometric_factor.box_feature_factor.layers[0][0].weight # 600 x 24
    ...
    hoi_dict = {hoi['id']: hoi['object']+'_'+hoi['verb'] for hoi in hoi_list}
    for i in range(len(hoi_list)):
        hoi_id = str(i+1).zfill(3)
        hoi_names.append(hoi_dict[hoi_id])
